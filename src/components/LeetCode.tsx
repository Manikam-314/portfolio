...
export default function LeetCode() {
  return (
    <section id="leetcode" className="section-ice py-16 sm:py-28 relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center">
          ...
          <motion.div
            ...
            className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-card border border-[rgba(180,210,230,.4)] dark:border-slate-800 shadow-ice-md p-6 sm:p-10 flex flex-col items-center text-center">
            ...
            <div className="relative w-36 h-36 sm:w-44 sm:h-44 mb-6 sm:mb-8">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl sm:text-4xl font-extrabold text-gray-950 dark:text-white">200+</span>
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1">Solved</span>
              </div>
            </div>

            {/* Difficulty split */}
            <div className="grid grid-cols-3 gap-2 sm:gap-4 w-full border-t border-[rgba(180,210,230,.4)] dark:border-slate-800 pt-5 sm:pt-6">
              {[
                ...
                <div key={d.label}>
                  <p className={`text-lg sm:text-xl font-extrabold ${d.color}`}>{d.value}</p>
                  <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-wide mt-0.5">{d.label}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6 sm:gap-8">
            <div>
              <h3 className="text-lg sm:text-xl font-extrabold text-gray-950 dark:text-white mb-3 sm:mb-4 tracking-tight">
                Algorithms → Production Engineering
              </h3>
              <div className="flex flex-col gap-3 sm:gap-4 text-[15px] sm:text-[17px] text-slate-500 dark:text-slate-400 leading-relaxed">
