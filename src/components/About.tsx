...
export default function About() {
  return (
    <section id="about" className="relative section-white py-28 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-6">
            <motion.h2
              ...
              className="text-3xl sm:text-[48px] font-extrabold tracking-tight text-gray-950 dark:text-white leading-[1.1] mb-8">
              Building systems that handle the hard parts
            </motion.h2>
            <motion.div
              initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.1 }}
              className="flex flex-col gap-4 text-[17px] text-slate-500 dark:text-slate-400 leading-relaxed">
              <p>
          ...
          <div className="lg:col-span-6 grid grid-cols-2 gap-5">
            {[
              ...
              <motion.div key={s.label}
                ...
                className="bg-[#F2F8FD] dark:bg-slate-900 rounded-card p-6 border border-[rgba(180,210,230,.4)] dark:border-slate-800 shadow-ice-sm hover:shadow-ice-md hover:-translate-y-0.5 transition-all">
                <p className="text-3xl font-extrabold text-ice-500 dark:text-ice-400 tracking-tight mb-1">{s.value}</p>
          ...
          {pillars.map((p, i) => {
            ...
            return (
              <motion.div key={p.title}
                ...
                className="bg-white dark:bg-slate-900 rounded-card p-7 border border-[rgba(180,210,230,.35)] dark:border-slate-800 shadow-ice-sm hover:shadow-ice-md hover:-translate-y-1 transition-all">
                <div className={`inline-flex p-3 rounded-2xl ${p.bg} mb-5`}>
